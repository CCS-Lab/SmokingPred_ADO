...
run_model_with_loocv <- function(data, config) {
  ...
  # Run CV with participant-level folds 
  subjects <- unique(data$subject)
  folds <- create_folds(subjects, type = "random", 
                        k = CV_K, m_rep = CV_REP)
  fit_r2_train <- c()
  fit_r2_test <- c()
  fit_mse_train <- c()
  ...
  for (fold in folds) {
    subj_train <- subjects[fold]
    data_train <- data_all %>% dplyr::filter(subject %in% subj_train)
    data_test  <- data_all %>% dplyr::filter(!(subject %in% subj_train))
